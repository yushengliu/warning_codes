...
@Description: 产生事件预警数据文件 —— 用服務器的定是程序來啟動
"""
...
# 找出当前正在跟踪的事件
def get_running_trace_seed_list(events_type, record_now, limit=0, update=True):
    # 2018/8/23 注掉， 应该没问题
    ...
    if record_now:
        ...
        # 2018/9/3 沁哥更新事件状态
        if update:
            sql = "SELECT a.events_head_id, a.gov_id, a.gov_name, a.key_word_str, a.start_date,a.sync_time, a.search_cnt from public_sentiment_trace_seed_table a,(select events_head_id, max(sync_time) as sync_time from public_sentiment_trace_seed_table WHERE (trace_state=4 and spider_sync_time>=(current_date - interval'3day')) OR trace_state in (2,7,9) group by events_head_id) b WHERE a.sync_time = b.sync_time and a.events_head_id=b.events_head_id AND a.events_type = %d ORDER BY start_date DESC;"%events_type
